...
<body>
    <div class="print-container">
        ...
        <div class="button-container no-print">
            ...
            <a href="{{ route('admin.barang-masuk.index') }}" class="back-button">
        ...
        <div class="header">
            ...
            <div class="date-range">
                @if(isset($tanggalAwal) && $tanggalAwal && isset($tanggalAkhir) && $tanggalAkhir)
                ...
                @elseif(isset($tahun) && $tahun)
                    Periode: Tahun {{ $tahun }}
                @elseif(isset($bulan) && $bulan)
                    Periode: {{ \Carbon\Carbon::create()->month($bulan)->format('F') }}
                @else
                    Periode: Semua Data
                @endif
            </div>
        ...
        <div class="info-summary">
            ...
            <div class="info-item">
                ...
                <div class="value">Rp {{ number_format($totalNilai ?? 0, 0, ',', '.') }}</div>
            </div>
            <div class="info-item">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="text-align: right;">Rp {{ number_format($item->nilai_total, 0, ',', '.') }}</td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="12" style="text-align: center;">Tidak ada data barang masuk</td>
                    </tr>
                    @endforelse
                </tbody>
                @if(isset($barangMasuks) && count($barangMasuks) > 0)
                <tfoot>
                    <tr>
                        ...
                        <td></td>
                        <td></td>
                        <td style="text-align: right;"><strong>Rp {{ number_format($totalNilai ?? 0, 0, ',', '.') }}</strong></td>
                    </tr>
                </tfoot>
                @endif
            </table>
    ...
    <script>
        ...
        // Fungsi untuk kembali ke halaman sebelumnya (alternatif)
        function goBack() {
            window.history.back();
        }
        
        // Tambahan: shortcut keyboard Ctrl+B untuk kembali
        document.addEventListener('keydown', function(event) {
            // Ctrl + B untuk kembali
            if (event.ctrlKey && event.key === 'b') {
                event.preventDefault();
                window.location.href = '{{ route("admin.barang-masuk.index") }}';
            }
        });
    </script>
</body>
